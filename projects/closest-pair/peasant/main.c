...
Pair* getClosestPair(Space* spaceX, Space* spaceY, int indent) {
  indent++;
  printf("%*s", indent * 3, "");
  printf("getClosestPair called with a space of %d points.\n", spaceX->count);
  ...
  if (spaceX->count == 2) {
    ...
    printf("%*s", indent * 3, "");
    printf("-> Count is 2 -- Returning as pair. (%d, %d) -> (%d, %d) = %f\n",
      a->x, a->y, b->x, b->y, distance
    );

    return createPair(a, b, distance);
  }

  // Do brute-force if n == 3
  if (spaceX->count == 3) {
    Pair* closestPair = getClosestPairByBruteForce(spaceX);
  ...
  printf("%*s", indent * 3, "");
  printf("-> Count is > 3 (%d) -- Dividing space into subspaces, left %d and right %d.\n",
    count, mid, count - mid
  );
...
Pair* findClosestPair(Space* space) {
  ...
  // Get closest pair.
  Pair* closestPair = getClosestPair(spaceX, spaceY, 0);
